...
public class GesturePassActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// setContentView(v);
		setContentView(R.layout.gesture_set_layout);//设置锁！
	...
	class ImageAdapter extends BaseAdapter {
		...
		public int change(StringBuffer sb) {//调用此函数直接改变密码并进行验证
			 int result=0;
			/*SharedPreferences store = GesturePassActivity.this.getSharedPreferences("angellpay", GesturePassActivity.this.MODE_WORLD_READABLE);
			//SharedPreferences.Editor editor = store.edit();
			String be= store.getString("gesturePass", "none");
			if(!be.equals("none"))
				beforString.append(be);
			if(beforString.length()==0 && sb.length()>=4)
				resetList(sb);
			 */	
			 if (sb.length() >= 4) {
				//合法的密码输入
				if (beforString.length() == 0) {//之前没有beforString（开机密码）
					beforString.append(sb.toString());//添加此输入sb于beforString
					tvMsg.setText("确认您的手势密码");//再输入一遍的提示
					
				
				} 
				else 
				{//beforString长度不是零
					if (beforString.toString().equals(sb.toString())) {//sb与beforString相同
						SharedPreferences preferences = GesturePassActivity.this.getSharedPreferences("angellpay", GesturePassActivity.this.MODE_WORLD_READABLE);
						SharedPreferences.Editor editor = preferences.edit();
						editor.putString("gesturePass", beforString.toString());//储存beforString为开机密码
						editor.commit();
						
						Intent intent=new Intent(GesturePassActivity.this, LockActivity.class);//启动LockActivity开启验证过程
						
						GesturePassActivity.this.startActivity(intent);
						//finish();
						onDestroy();
					}
					else//sb与beforString不同
					{
						//Toast.makeText(context, "手势密码错误,请再次输入!", 1).show();
						System.out.println("李佳隆测试到这里（两次设定结果不一样）");
						return -1;
					}
				}
			}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			LinearLayout layout = (LinearLayout) inflater.inflate(R.layout.gridview_item, null);
