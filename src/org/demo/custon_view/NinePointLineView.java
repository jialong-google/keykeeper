...
//			R.drawable.pattern_grid_focused);
	...
	protected void onDraw(Canvas canvas) {
		if(isDownIn&&isUpIn){
			...
			int textWidth=0;
			//设置密码
			if((show||isWrong)){//长度小于四show或密码改写错误isWrong
				text="设置的密码长度最小是4,请重绘";
				Rect rect=new Rect();
				if(isWrong){//长度不小于四，只是密码改写错误，立马切换text内容
					text="两次设定结果不相同，请输入初次修改的密码";
				}
...
//			canvas.drawText(text, (width-textWidth)/2, 40, textPaint);
			...
			//验证密码//////////////////////////
			else if((isWrong1||show)){
				Rect rect=new Rect();
				//text="设置的密码长度最小是4,请重绘!";
				if(isWrong1 ){
					text="密码错误,还可以再输入"+(inputCount--)+"次!";
					if(inputCount==3) text=" ";
				}
				textPaint.getTextBounds(text, 0, text.length(), rect);
				textWidth=rect.width();
				isWrong1=false;
	...
	private void handlingEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			if(inputCount<=0){//不允许再次输入！
				//SharedPreferences preferences = getContext().getSharedPreferences("angellpay", getContext().MODE_WORLD_READABLE);
				//SharedPreferences.Editor editor = preferences.edit();
				//editor.putString("gesturePass", null);////////销毁密码？
				//editor.commit();
...
//				Toast.makeText(getContext(), "输入成功", 1).show();
			...
			if(pass!=null && !pass.equals(lockString.toString())){
				isWrong1=true;//pass不为空 且pass不等于lockString 即错误
			}else{
				isWrong1=false;
			}
			if(adapter!=null)
				result=adapter.change(new StringBuffer(lockString.toString()));//用lockString来改写密码。
			
			if(result==-1){//change的结果不对。
				isWrong=true;//此时应该显示两次设定结果不相同
			}else{
				isWrong=false;
			}
			if(lockString.length()<4){
				show=true;//长度小于4的非法锁屏序列
			}
			else{
				show=false;
			}
